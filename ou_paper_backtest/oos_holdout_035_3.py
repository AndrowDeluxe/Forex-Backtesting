"""One more candidate risk profile requested (2026-08-07) as a possible final
choice: 0.35%/trade, 3% aggregate -- between the conservative (0.25%/2.5%) and
middle (0.5%/4%) profiles already tested. Same locked entry/exit, same genuine
2025+ holdout, same challenge rule check (max 3% single-day loss, 10% profit
target) as oos_holdout_challenge_profiles.py / oos_holdout_final_profiles.py."""
...
PROFILE = ("candidate_0.35pct_3pct", 0.0035, 0.03)
...
if __name__ == "__main__":
    ...
    df = run(universe_key)
    out_path = config.RESULTS_DIR / universe_key / "oos_holdout_035_3.csv"
