"""Two concrete risk profiles for a 100k funded-challenge account, on the genuine
2025+ holdout (fresh data, never touched by any sweep) -- requested (2026-08-07)
against the actual rules of the user's funded challenge: max 3% loss in a single
day, 10% total profit target. Locked baseline entry/exit (stop_sigma=3.0, no TP,
be_trigger_r=0.25) throughout -- only risk_pct and max_total_risk_pct vary.

  - "conservative" (0.25% / trade, 2.5% aggregate): matches the challenge's own
    max_total_risk_pct=2.5% precedent already running on the live bot's Konto 2.
  - "aggressive" (1% / trade, 10% aggregate): the other end the user asked for.

Also reports the worst single calendar-day % loss on the equity curve, since that's
what actually violates a "max 3%/day" challenge rule -- Max Drawdown (peak-to-
trough over the WHOLE curve) is a different, unrelated number and can look fine
while a single bad day still breaches the daily rule.
"""
...
PROFILES = [
    ("conservative_0.25pct_2.5pct", 0.0025, 0.025),
    ("aggressive_1pct_10pct", 0.01, 0.10),
]
...
def run(universe_key: str) -> pd.DataFrame:
    ...
    for label, risk_pct, cap in PROFILES:
        ...
        if not hit.empty:
            days_to_10pct = (hit.index[0] - eq.index[0]).days
...
if __name__ == "__main__":
    ...
    df = run(universe_key)
    out_path = config.RESULTS_DIR / universe_key / "oos_holdout_challenge_profiles.csv"
