"""Follow-up to research_gold_smc_reversal_cascade_concurrent.py (chat
2026-08-19): finer/wider re-sweep at the user's request - "teste gerne
nochmal verschiedene max Trades mit verschiedenen Exit parametern" and a
wider max_hold range (v1 only went to 96h; re-entry changes the
opportunity-cost logic of a long hold since new entries are no longer
blocked, so the optimal hold time might differ from the single-position
sweep's finding).

Sweeps max_hold_bars (now up to 168h) x tp_mode/tp_r x stop_atr x be x
max_concurrent (now a finer grid: 1,2,3,4,5,6,8,12, uncapped) - same IS-
select/OOS-validate discipline, using gold_smc_htf_ltf.concurrent_
backtest (simulate_trades_concurrent + simulate_account_reentry, with
the equity-blowup halt fix from the first round).
"""
...
MAX_CONCURRENT_CANDIDATES = [1, 2, 3, 4, 5, 6, 8, 12, None]
...
def main():
    ...
    sweep.to_csv(Path(__file__).resolve().parents[1] / "gold_smc_htf_ltf" / "_reversal_concurrent_v2_sweep.csv", index=False)
    eligible = sweep[sweep["n_trades"] >= MIN_IS_TRADES]
    print(f"\n{len(sweep)} combos tested, {len(eligible)} reach n>={MIN_IS_TRADES} IS trades")
    if eligible.empty:
        print("Stopping.")
        return
    top25 = eligible.sort_values("sharpe", ascending=False).head(25)
    print("\nTop 25 combos by IS Sharpe:")
    print(top25[["max_hold_h", "tp_mode", "tp_r", "stop_atr", "be", "max_concurrent", "n_trades", "win_rate", "profit_factor", "sharpe", "cagr", "max_drawdown"]].to_string(index=False))

    print("\nBest combo PER max_concurrent (by IS Sharpe, n>=15):")
    for mc in MAX_CONCURRENT_CANDIDATES:
        sub = eligible[eligible["max_concurrent"] == mc] if mc is not None else eligible[eligible["max_concurrent"].isna()]
        if sub.empty:
            print(f"  max_concurrent={mc}: no combo reaches n>={MIN_IS_TRADES}")
            continue
        b = sub.loc[sub["sharpe"].idxmax()]
        print(f"  max_concurrent={mc}  max_hold={b['max_hold_h']}h tp={b['tp_mode']}/{b['tp_r']} stop={b['stop_atr']} be={b['be']}  {fmt(b.to_dict())}")
    ...
    print("\n" + "=" * 78)
    print("4. OUTLIER CHECK + max_concurrent SENSITIVITY at winning exit config (OOS)")
    print("=" * 78)
    if not oos_raw.empty:
        oos_r_sorted = oos_raw.sort_values("return_pct", ascending=False)
        without_best_raw = oos_raw.drop(index=oos_r_sorted.index[0])
        s_wo = summarize_concurrent(without_best_raw, sig_oos.index, max_conc)
        print(f"  OOS with best single trade removed: {fmt(s_wo)}  total_return={s_wo['total_return']:+.1%}")
    for mc in MAX_CONCURRENT_CANDIDATES:
