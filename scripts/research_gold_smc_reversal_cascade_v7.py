"""Follow-up to research_gold_smc_reversal_cascade_v6.py (chat 2026-08-19).
User provided real chart examples showing a trendline-break pattern and
clarified: (1) the SAME diagonal-break mechanism, mirrored, should catch
REVERSALS as close to the actual high/low as possible (new m15_entry_mode
="trendline" in reversal_cascade.py, mirroring continuation.py's), and
(2) specifically the LTF entry (for a better risk/reward) should use the
"2x over the same point" repeat-sweep idea (new m15_entry_mode=
"repeat_sweep", mirroring continuation.py's repeat_sweep entry_variant).

Tests m15_entry_mode in {sweep, trendline, repeat_sweep} x exit params,
H4/H1 stage fixed at the standing-best config (h4_confirm=30, h1_valid=24,
ema_reject, sweep_mode=double/tol=0, h1_bos_min=2, no inducement/
magnitude/volume/age - none improved on baseline per v3-v5).
"""
...
ENTRY_MODES = ["sweep", "trendline", "repeat_sweep"]
...
def main():
    ...
    print("\nRunning pipeline once per m15_entry_mode ...")
    signaled_by_mode = {}
    ...
    for mode, sig in signaled_by_mode.items():
        ...
        for stop_mult in STOP_ATR_CANDIDATES:
            for be in BE_CANDIDATES:
                ...
                rows.append({"mode": mode, "stop_atr": stop_mult, "be": be, **s})
    ...
    print(top15[["mode", "stop_atr", "be", "n_trades", "win_rate", "profit_factor", "sharpe", "cagr"]].to_string(index=False))

    print("\nBest combo PER mode (by IS Sharpe, n>=15):")
    for mode in ENTRY_MODES:
        sub = eligible[eligible["mode"] == mode]
        if sub.empty:
            print(f"  {mode:<15}: no combo reaches n>={MIN_IS_TRADES}")
            continue
        b = sub.loc[sub["sharpe"].idxmax()]
        print(f"  {mode:<15} stop={b['stop_atr']} be={b['be']}  {fmt(b.to_dict())}")

        # also show this mode's own OOS, regardless of whether it's the overall winner
        mode_sig_full = signaled_by_mode[mode]
        mode_sig_oos = mode_sig_full[mode_sig_full.index >= SPLIT]
        mode_be = None if pd.isna(b["be"]) else float(b["be"])
        mode_cfg = BacktestConfig(spread_bps=SPREAD_BPS, stop_atr_mult=float(b["stop_atr"]), use_vwap_target=True, breakeven_trigger_r=mode_be, max_hold_bars=MAX_HOLD_BARS)
        mode_oos_trades = simulate_trades(mode_sig_oos, mode_cfg)
        mode_oos_stats = summarize(mode_oos_trades, mode_sig_oos.index)
        print(f"    -> OOS: {fmt(mode_oos_stats)}")

    best = eligible.loc[eligible["sharpe"].idxmax()]
    print(f"\nStandout combo overall (highest IS Sharpe, n>={MIN_IS_TRADES}):")
    print(f"  mode={best['mode']}  stop={best['stop_atr']}  be={best['be']}")
    ...
    print("=" * 78)
    sig_full = signaled_by_mode[mode]
